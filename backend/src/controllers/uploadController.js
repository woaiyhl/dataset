...
async function uploadImmediate(req, res) {
  ...
  parser.on('readable', () => { let r; while ((r = parser.read())) { records.push(r); if (!headers) headers = Object.keys(r) } })
  parser.on('error', (err) => res.status(400).json({ error: 'CSV parse error', detail: err.message }))
  parser.on('end', () => {
    try {
      ...
      const candidates = headers.map((h) => {
        let ok = 0
        for (let i = 0; i < sampleSize; i++) { const v = records[i]?.[h]; if (v !== undefined && isValidDate(v)) ok++ }
      ...
      nameHints.forEach((h) => { const c = candidates.find((x) => x.h === h); if (c) c.score += 0.2 })
      candidates.sort((a, b) => b.score - a.score)
      let timeKey = candidates[0]?.score >= 0.5 ? candidates[0].h : null
      if (!timeKey) return res.status(400).json({ error: 'No time-like column found', detail: '请确保至少一列为可解析的时间格式，例如 ISO 时间戳' })
      const seriesKeys = headers.filter((h) => h !== timeKey).filter((h) => {
        let cnt = 0
        for (let i = 0; i < sampleSize; i++) { const v = Number(records[i]?.[h]); if (Number.isFinite(v)) cnt++ }
        return sampleSize ? cnt / sampleSize >= 0.3 : true
      })
      if (seriesKeys.length === 0) return res.status(400).json({ error: 'No numeric series columns', detail: '未检测到数值型列，请检查数据格式' })
      ...
      for (const r of records) {
        ...
        for (const k of seriesKeys) { const v = Number(r[k]); point[k] = Number.isFinite(v) ? v : null }
        data.push(point)
      }
      const stats = {}
...
async function uploadStream(req, res) {
  if (!req.file) return res.status(400).json({ error: 'No file uploaded' })
  const id = genId()
  uploads.set(id, { id, name: req.file.originalname, size: req.file.size, status: 'processing', percent: 0, result: null, createdAt: Date.now() })
  res.json({ jobId: id })
...
function uploadProgressSSE(req, res) {
  ...
  const send = () => {
    ...
    if (j.status === 'done' || j.status === 'failed') { clearInterval(timer); res.end() }
  }
