...
const headers = [
  'id','_id','localDateTime','t','dataTimeStamp','device_id','assetnumber','MotorCurrent','SpeedOfAgitator','Reducer1stShaftTemp','motorDrBearingTemp','motorNdrBearingTemp','motorWindingTemp1','motorWindingTemp2','motorWindingTemp3','OperationIndication'
]
...
for (let i = 0; i < 1000; i++) {
  ...
  const localDateTime = new Date(start.getTime() + (i + 49) * 1000).toISOString().replace('T', ' ').slice(0, 23)
  ...
  const row = [id,_id,localDateTime,t,dataTimeStamp,device_id,assetnumber,MotorCurrent,SpeedOfAgitator,Reducer1stShaftTemp,motorDrBearingTemp,motorNdrBearingTemp,motorWindingTemp1,motorWindingTemp2,motorWindingTemp3,OperationIndication]
  lines.push(row.join(','))
}
