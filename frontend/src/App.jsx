...
function App() {
  ...
  const onFileListChange = useCallback((list) => {
    ...
    setPayloadByUid((prev) => {
      const next = {}
      Object.keys(prev).forEach((k) => { if (uids.has(k)) next[k] = prev[k] })
      return next
    })
  ...
  const handleRemoveByUid = useCallback((uid) => {
    const next = fileList.filter((f) => f.uid !== uid)
    const uids = new Set(next.map((f) => f.uid))
    setFileList(next)
    setPayloadByUid((prev) => {
      const out = {}
      Object.keys(prev).forEach((k) => { if (uids.has(k)) out[k] = prev[k] })
      return out
    })
    if (selectedUid === uid) {
  ...
  return (
    <Layout className="app-wrapper" style={{ minHeight: '100vh' }}>
      <Layout.Header className="header">
        <Space align="center" wrap style={{ width: '100%', justifyContent: isMobile ? 'flex-start' : 'space-between' }}>
          <Typography.Title level={3} style={{ margin: 0, color: '#fff' }}>时序数据可视化</Typography.Title>
        </Space>
      </Layout.Header>
        ...
        <Row gutter={[16, 0]} className="main-row" align="stretch" wrap={false}>
          <Col xs={24} className="sidebar-fixed">
            <Card className="panel sidebar-card" title="数据设置">
              <UploadCsv onSuccess={handleUploadSuccess} onParsed={onParsed} onFileListChange={onFileListChange} onRemove={onRemove} existingNames={fileList.map((f) => f.name)} />
              {fileList.length === 0 ? (
                <div>暂无文件</div>
              ) : (
                <Space orientation="vertical" style={{ width: '100%' }}>
                  <div className="file-summary">
                    <div>已上传文件：<span className="file-count">{fileList.length}</span></div>
                  </div>
                  <div className="file-list">
                    {fileList.map((f) => (
                      ...
                      >
                        <span className={`file-name${selectedUid === f.uid ? ' selected' : ''}`}>{f.name}</span>
                        <span
